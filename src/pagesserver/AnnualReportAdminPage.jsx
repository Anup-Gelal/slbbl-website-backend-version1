...
const AdminAnnualReports = () => {
  ...
  const fetchReports = async () => {
    try {
      const res = await axios.get(`${API_BASE}/admin/annual-reports`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`${API_BASE}/admin/annual-reports/${editingId}`, formData, {
          headers: {
        ...
        await axios.post(`${API_BASE}/admin/annual-reports`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "multipart/form-data",
          },
  ...
  const startEditing = (r) => {
    setForm({
      ...
      file: null, // keep existing unless changed
    });
    setEditingId(r.id);
    setError("");
    setSuccess("");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE}/admin/annual-reports/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-white shadow rounded">
      <h2 className="text-2xl font-bold text-blue-700 mb-4">Manage Annual Reports</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 mb-8" encType="multipart/form-data">
        <div>
          ...
          <input
            ...
            placeholder="e.g. 15th Annual Report FY 2079/080"
          />
        </div>
