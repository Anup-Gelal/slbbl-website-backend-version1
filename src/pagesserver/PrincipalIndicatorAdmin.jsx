...
const AdminPrincipalIndicators = () => {
  ...
  const fetchReports = async () => {
    try {
      const res = await axios.get(`${API_BASE}/admin/principal-indicators`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`${API_BASE}/admin/principal-indicators/${editingId}`, formData, {
          headers: {
        ...
        await axios.post(`${API_BASE}/admin/principal-indicators`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "multipart/form-data",
          },
      ...
      setError(err.response?.data?.error || "Failed to save indicator.");
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE}/admin/principal-indicators/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-white shadow rounded">
      <h2 className="text-2xl font-bold text-green-700 mb-4">Manage Principal Indicators</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 mb-8" encType="multipart/form-data">
        <div>
          ...
          <input
            ...
            placeholder="e.g. Principal Indicators FY 2080.081"
          />
        </div>
        ...
        <div className="flex gap-4">
          ...
          >
            {editingId ? "Update" : "Add"} Indicator
          </button>
          {editingId && (
            <button
              type="button"
...
export default AdminPrincipalIndicators;
