...
const AllBranches = () => {
  ...
  const fetchBranches = async () => {
    ...
    setError("");
    try {
      const res = await axios.get("https://slbbl-website-backend-version1.onrender.com/api/v1/branches");
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto bg-gray-50 min-h-screen">
      ...
      {!loading && paginatedBranches.length > 0 ? (
        ...
        !loading && <p className="text-center text-green-600">No results found</p>
      )}
...
export default SectionWrapper(AllBranches, "allbranches");
...
import { Button } from "@/components/ui/button";
import { ChevronLeft, ChevronRight, Search } from "lucide-react";
import { ourBranches } from "../constants";
import { SectionWrapper } from "@/hoc";
...
const AllBranches = () => {
  ...
  const filteredBranches = ourBranches.filter((branch) =>
    branch.name.toLowerCase().includes(search.toLowerCase())
  );
      ...
      {paginatedBranches.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-center items-center">
          {paginatedBranches.map((branch, idx) => (
            ...
            >
              <CardContent>
                <h2 className="text-lg font-bold text-blue-700">{branch.id}.{branch.name}</h2>
