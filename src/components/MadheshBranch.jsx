...
export default SectionWrapper(MadheshBranch, "madheshbranch");
...
import { Button } from "@/components/ui/button";
import { ChevronLeft, ChevronRight, Search } from "lucide-react";
import { madheshBranches } from "../constants";
import { SectionWrapper } from "@/hoc";
...
const MadheshBranch = () => {
  ...
  const filteredBranches = madheshBranches.filter((branch) =>
    branch.name.toLowerCase().includes(search.toLowerCase())
  );
      ...
      {paginatedBranches.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-center items-center">
          {paginatedBranches.map((branch, idx) => (
            <Card key={idx} className="p-4 bg-white shadow-lg rounded-lg border border-gray-200 transition-all hover:shadow-xl">
              <CardContent>
