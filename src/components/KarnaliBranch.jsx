...
export default SectionWrapper(KarnaliBranch, "karnalibranch");
...
import { Button } from "@/components/ui/button";
import { ChevronLeft, ChevronRight, Search } from "lucide-react";
import { karnaliBranches } from "../constants";
import { SectionWrapper } from "@/hoc";
...
const KarnaliBranch = () => {
  const [search, setSearch] = useState("");
  const [province, setProvince] = useState("karnali");
  const [page, setPage] = useState(0);

  const filteredBranches = karnaliBranches.filter((branch) =>
    branch.name.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="p-6 max-w-5xl mx-auto bg-gray-100 min-h-screen">
      ...
      <div className="flex gap-2 mb-6 items-center">
        <Input
          ...
          className="p-3 w-full border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500"
        />
        <Button variant="outline" className="px-4 py-3">
          <Search size={18} />
        </Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-center items-center">
        {paginatedBranches.length > 0 ? (
          paginatedBranches.map((branch, idx) => (
            <Card key={idx} className="p-4 bg-white shadow-md rounded-lg border border-gray-200">
              <CardContent>
          ...
          <p className="text-center text-gray-600 col-span-full">No results found</p>
        )}
      </div>
